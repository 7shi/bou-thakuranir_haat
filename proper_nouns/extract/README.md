...
`make -C proper_nouns/extract` runs the full extract-then-translate sequence
(English, then Japanese and Hindi via `translate.py`) with default settings -
see [`Makefile`](Makefile). For anything else, run the scripts directly:

### Extracting Proper Nouns

```bash
uv run proper_nouns/extract/extract.py SOURCE.md -f SOURCE_LANG -t TARGET_LANG -m MODEL -w WORK.jsonl -o OUTPUT.tsv
```
...
```bash
uv run proper_nouns/extract/translate.py [INPUT.tsv] -f SOURCE_LANG -i INTERMEDIATE_LANG -t TARGET_LANG -m MODEL -o OUTPUT.tsv
```
...
1. **Extract Bengali → English proper nouns (creates initial TSV):**
   ```bash
   uv run proper_nouns/extract/extract.py all-bn.md -f bengali -t english -m google:gemini-2.5-pro -w proper_nouns/extract/en.jsonl -o proper_nouns/extract/all.tsv
   ```

2. **Add Japanese translations to existing TSV:**
   ```bash
   uv run proper_nouns/extract/translate.py -f bengali -i english -t japanese -m google:gemini-2.5-pro -o proper_nouns/extract/all.tsv
   ```

3. **Add another language (e.g., Chinese):**
   ```bash
   uv run proper_nouns/extract/translate.py -f bengali -i english -t chinese -m google:gemini-2.5-pro -o proper_nouns/extract/all.tsv
   ```
...
The final `proper_nouns/extract/all.tsv` will contain proper nouns in all processed languages with proper normalization and consolidation.
