...
the difference is where the drop threshold sits:
  ...
  --verdicts 10: eleven-level verdict (an integer 0-10). Phase 1 records the
   ...
   --verdicts 5d: five-axis verdict (five integers 0-10, one per relevance axis).
            ...
            entity, causal, reference, thematic) so that a chapter scoring 0 on
            one axis can still earn points on another — the lever for the
            gold-scored-0 floor that single-axis variants (10/100) cannot reach.
            Phase 2 keeps a chapter when the five-axis sum is non-zero (absence
            is the failure, surfeit is not). Phase 1 records the five raw scores
            per (chapter, question) pair so the floor can be inspected before
            any Phase 2 answering; run with --phase1 to produce filter5d.tsv.
            Uses structured output (a pydantic schema) via
            create_json_descriptions_prompt, unlike the plain-text single-token
            replies of the other variants.
...
# The five relevance axes for the `--verdicts 5d` variant. Each axis is scored
# 0-10 independently; the caller sums them and keeps a chapter when the sum is
# non-zero. Decomposing the judgement across orthogonal axes prevents the
# all-zero verdict that single-axis scoring collapses to on indirectly-relevant
# chapters (a chapter scoring 0 on `factual` can still earn points on
# `reference` or `causal`). The order here fixes the TSV column order.
AXES = ("factual", "entity", "causal", "reference", "thematic")
VERDICT_5D_HEADER = "chapter	question_id	" + "	".join(AXES)


class AxisScores(BaseModel):
    """Five independent relevance scores (each 0-10) for one (chapter, question) pair.

    The sum is computed by the caller, not generated by the model: it is pure
    arithmetic over the five fields, so asking the model for it would only add
    a field it could get wrong.
    """

    factual: int = Field(
        ..., ge=0, le=10,
        description="facts that directly answer the question",
    )
    entity: int = Field(
        ..., ge=0, le=10,
        description="the question's characters, objects, or places appear or act",
    )
    causal: int = Field(
        ..., ge=0, le=10,
        description="causes, prerequisites, or consequences of the question's event",
    )
    reference: int = Field(
        ..., ge=0, le=10,
        description="cross-references, callbacks, or foreshadowing linked to the question",
    )
    thematic: int = Field(
        ..., ge=0, le=10,
        description="themes, symbols, or motifs related to the question",
    )
...
def read_verdict_5d_tsv(path: Path):
    """Yield (chapter, question_id, scores) tuples from a 5d verdict TSV.

    `scores` is a dict mapping each axis name in `AXES` to its integer score.
    Skips the header row (first line whose first field is `chapter`) and blank
    lines, mirroring `read_verdict_tsv` so the same resume logic works.
    """
    with open(path, encoding="utf-8") as f:
        for i, line in enumerate(f):
            line = line.rstrip("\n")
            ...
            ch = int(fields[0])
            qid = int(fields[1])
            scores = {axis: int(v) for axis, v in zip(AXES, fields[2:])}
            yield ch, qid, scores
...
def classify_chapter_5d(question: str, chapter: int, chapter_text: str,
                        model: str) -> AxisScores:
    """Rate a chapter's relevance on five independent axes (each 0-10).

    Unlike `classify_chapter_score`, which collapses relevance onto one scale,
    this asks for five orthogonal axis scores in a single structured-output
    call. A chapter that scores 0 on `factual` can still earn points on
    `reference`, `causal`, etc., so the five-axis decomposition structurally
    resists the all-zero verdict that single-axis scoring collapses to on
    indirectly-relevant cross-reference chapters. The caller sums the five
    scores and keeps the chapter when the sum is non-zero.

    Structured output is used (a pydantic schema), and the JSON field
    description is injected as a third message via
    `create_json_descriptions_prompt` (same pattern as translate_questions.py).
    `sum` is intentionally not a field — it is computed in logic. On a parse or
    validation failure the call retries up to 3 times; a still-failing reply
    defaults to all zeros with a warning, so the failure stays visible rather
    than being masked as a fabricated relevance score.
    """
    context = f"Chapter {chapter} text:\n{chapter_text}"
    prompt = (
        f"Rate the chapter text above on FIVE independent relevance axes for "
        f"answering the question below. Each axis is an integer 0-10 "
        f"(0 = absent, 10 = central to the question):\n"
        f"  - factual: facts that directly answer the question.\n"
        f"  - entity: the question's characters, objects, or places appear or act.\n"
        f"  - causal: causes, prerequisites, or consequences of the question's event.\n"
        f"  - reference: cross-references, callbacks, or foreshadowing linked to the question.\n"
        f"  - thematic: themes, symbols, or motifs related to the question.\n"
        f"A chapter that does NOT directly answer can still score non-zero on "
        f"the other axes — score each axis independently.\n\n"
        f"Question: {question}"
    )
    json_description = create_json_descriptions_prompt(AxisScores)
    max_retries = 3
    for attempt in range(max_retries + 1):
        try:
            result = generate_with_schema(
                [context, prompt, json_description],
                AxisScores,
                model=model,
                include_thoughts=False,
                show_params=False,
            )
            return AxisScores(**json.loads(result.text))
        except (json.JSONDecodeError, ValidationError, ValueError) as e:
            if attempt < max_retries:
                print(f"  5d parse/validation failed ({e}) — retrying ({attempt + 1}/{max_retries})")
            else:
                print(f"  5d still failing after {max_retries} retries — "
                      f"defaulting to all zeros ({e})")
    return AxisScores(factual=0, entity=0, causal=0, reference=0, thematic=0)
...
def _parse_verdicts(value: str):
    """Parse --verdicts: ints 2/3/10/100 stay ints, the string "5d" stays a string.

    `argparse` applies `type` only to command-line strings, not to the `default`
    (which is the int 3), so the default bypasses this function cleanly.
    """
    if value == "5d":
        return "5d"
    ivalue = int(value)
    if ivalue not in (2, 3, 10, 100):
        raise argparse.ArgumentTypeError(f"invalid --verdicts value: {value!r}")
    return ivalue
...
def main():
    ...
    parser.add_argument("--verdicts", type=_parse_verdicts, default=3,
                        choices=[2, 3, 10, 100, "5d"],
                        help="relevance granularity: 3 = yes/maybe/no (keep yes+maybe, default), "
                             "2 = yes/no (keep yes only), 10 = integer 0-10 (Phase 1 only), "
                             "100 = integer 0-100 (Phase 1 only), "
                             "5d = five-axis 0-10 each, keep if sum>0 (Phase 1 only)")
    ...
    if args.phase1:
        ...
        # Resume: collect done (qid, chapter) pairs. The 5d variant stores
        # five axis scores per pair (a dict), the others a single verdict
        # string — only the key membership is checked below, so the value type
        # is heterogeneous.
        done: dict[tuple[int, int], object] = {}
        if verdict_path.exists():
            if args.verdicts == "5d":
                for ch, qid, scores in read_verdict_5d_tsv(verdict_path):
                    done[(qid, ch)] = scores
            else:
        ...
        # appends do not duplicate it. The 5d variant has one column per axis.
        header = VERDICT_5D_HEADER if args.verdicts == "5d" else VERDICT_HEADER
        ...
        with open(verdict_path, "a", encoding="utf-8") as ckpt_f:
            for ch in all_chapter_ids:
                ...
                for qid, q in enumerate(questions, start=1):
                    ...
                    if args.verdicts == "5d":
                        scores = classify_chapter_5d(
                            question_text, ch, chapter_text, args.model)
                        done[(qid, ch)] = scores.model_dump()
                        ckpt_f.write(
                            f"{ch}\t{qid}\t{scores.factual}\t{scores.entity}\t"
                            f"{scores.causal}\t{scores.reference}\t{scores.thematic}\n"
                        )
                    elif args.verdicts in (10, 100):
