...
| --- | --- | --- | --- |
| Vector k=5 | 40/50 (0.840) | 38/50 (0.810) | 標準的な密ベクトル検索 (k=5) |
| Vector k=10 | 45/50 (0.930) | 43/50 (0.900) | 標準的な密ベクトル検索 (k=10) |
| Vector-line k=5 | 36/50 (0.810) | 36/50 (0.800) | 行レベルの密ベクトル検索 (k=5) |
| Vector-line k=10 | 41/50 (0.890) | 41/50 (0.860) | 行レベルの密ベクトル検索 (k=10) |
| V-hybrid k=5 | 40/50 (0.880) | 42/50 (0.890) | Segment ∪ Line 密ベクトル集合和 (k=5) |
| V-hybrid k=10 | 43/50 (0.910) | 43/50 (0.900) | Segment ∪ Line 密ベクトル集合和 (k=10) |
| Hybrid k=5 | 42/50 (0.900) | 45/50 (0.930) | Dense ∪ BM25 集合和 (k=5) |
| Hybrid k=8 | 45/50 (0.930) | 46/50 (0.950) | Dense ∪ BM25 集合和 (k=8) |
| Hybrid k=10 | 47/50 (0.960) | 45/50 (0.930) | Dense ∪ BM25 集合和 (k=10) |
| Extract | 40/50 (0.850) | 41/50 (0.860) | チャプターごとの要約ベース判定 |
| Filter2 | 36/50 (0.800) | 40/50 (0.840) | LLM-as-retriever (二値判定: yes/no) |
| Filter3 | 46/50 (0.940) | 43/50 (0.890) | LLM-as-retriever (三値判定: yes/maybe/no) |
| Ceiling | 49/50 (0.990) | 48/50 (0.980) | 完璧な検索の上限値 (正解チャプターを直接入力) |
...
> [!IMPORTANT]
> **実用的な最適解**
> 英語においては `Hybrid k=10` が最高精度 **0.960** を、日本語においては `Hybrid k=8` が最高精度 **0.950** を達成しており、いずれも **Dense ∪ BM25 (Union)** の組み合わせが実用上の最適解です。
...
* **日本語版 k=8 のスイートスポットと英語版 k=10 の比較:** 日本語版では、`k=10` にするとコンテキストの肥大化による回答合成エラー（Q34）が生じるため、`k=8` が **46/50 (0.950)** をマークする最適なスイートスポットとなりました。一方、英語版ではコンテキスト増大による合成エラーの影響が少なく、`Hybrid k=10` が最高精度 **47/50 (0.960)** を維持しており、`Hybrid k=8` は **45/50 (0.930)** となっています。
...
* **Ceiling の実証:** Ceiling ラン（正解チャプターをそのまま渡す）では 0.990 (英語) と 0.980 (日本語) のスコアを達成し、適切なコンテキストさえ与えられれば LLM の読解力はほぼ完璧であることが証明されました。残る3件（英語 Q48、日本語 Q29・Q36）はいずれも複数要素を並べる cross 質問で要素を取りこぼした網羅性の不足であり、読み違いではありません。
* **Extract の失敗原因:** Extract 手法での失点は、回答生成時の失敗ではなく、そのほとんどが Phase 1 の偽陰性（False negative: 要約が正解チャプターを切り捨ててしまうこと）によるものです。
* **言語による差異は小さい:** 使用する言語が精度に与える影響はわずかで、ほとんどの手法で英語と日本語の総計スコアの差は2問以内に収まり、最大でも4問（Filter2）です。同じ埋め込みモデルを使用しているため、検索で取りこぼす箇所も同一です。
...
### Jev による判定 (Jev as the Judge) — [jev/README.md](jev/README.md) & [JEV.md](JEV.md)

* **qwen の判定は甘い:** `ollama:qwen3.6` による判定では、複数モデルの Ceiling 比較で 98〜100 に張り付いています。TypeSafe の Jev (`jev-1.13.0`) に同じルーブリックで Choice を1問聞くと判定は厳しくなり、増えた *partial* の中身は文体ではなく、複数要素の正解の一部を取りこぼしたものです。qwen が *correct* とした「回答できない」という回答も1件含まれます。
* **上位を分離できる:** Ceiling の6モデル（英語）で、qwen が 100 とした3モデルは Jev では 93〜95 に下がり、qwen3.6（87）との差も広がります。
...
* **`results-<lang>/` の再判定:** Jev では全手法のスコアが 0.03〜0.12 下がり、その差はほぼすべて、複数要素を問う cross 質問で qwen の *correct* が Jev の *partial* になったものです。single 質問はほぼ変わりません。Ceiling は 0.960（英語）/ 0.900（日本語）に下がり、上位の Hybrid と Filter3 の差は1問以内に縮まります。上記の結果は qwen の判定のままです。
...
3. **最適戦略の言語間での一致:**
   * 両言語ともに、**Dense ∪ BM25 Union** (`Hybrid`) が最善のアプローチです（英語: **0.960** (k=10), 日本語: **0.950** (k=8)）。単一の密ベクトル検索の限界を効果的に打破し、固有名詞や特徴的な用語を回収することで Ceiling に最も近づきました。
