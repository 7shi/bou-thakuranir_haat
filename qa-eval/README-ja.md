...
| Vector-line k=5 | 35/50 (0.800) | 36/50 (0.800) | 行レベルの密ベクトル検索 (k=5) |
| Vector-line k=10 | 41/50 (0.890) | 41/50 (0.860) | 行レベルの密ベクトル検索 (k=10) |
| V-hybrid k=5 | 40/50 (0.880) | 42/50 (0.890) | Segment ∪ Line 密ベクトル集合和 (k=5) |
| V-hybrid k=10 | 43/50 (0.910) | 44/50 (0.910) | Segment ∪ Line 密ベクトル集合和 (k=10) |
...
| Filter2 | 36/50 (0.790) | 41/50 (0.850) | LLM-as-retriever (二値判定: yes/no) |
| Filter3 | 46/50 (0.940) | 43/50 (0.890) | LLM-as-retriever (三値判定: yes/maybe/no) |
| Ceiling | 49/50 (0.990) | 48/50 (0.980) | 完璧な検索の上限値 (正解チャプターを直接入力) |
| GraphRAG local | 27/50 (0.650) | 28/50 (0.640) | Microsoft GraphRAG (ローカルエンティティ検索) |
...
* **言語による差異は小さい:** 使用する言語が精度に与える影響はわずかで、ほとんどの手法で英語と日本語の総計スコアの差は2問以内に収まり、最大でも5問（Filter2）です。同じ埋め込みモデルを使用しているため、検索で取りこぼす箇所も同一です。

### GraphRAG — [graphrag-en/README.md](graphrag-en/README.md) & [graphrag-ja/README.md](graphrag-ja/README.md)

* **回答生成の崩壊:** GraphRAG の `local` 検索は英語 0.650 / 日本語 0.640 と、両言語で単純なベクトル検索を下回りました。再現率（Recall）は高い（英語 0.860 / 日本語 0.880）ものの、適合率（Precision）が崩壊しており（英語 0.135 / 日本語 0.239）、コンテキストにノイズが溢れて回答生成の失敗（synthesis failure）を引き起こしました。
* **構造的強みは言語間で再現しない:** 英語ではエンティティの関係性を示す質問（Q26/Q28）に対してパッセージのコンテキストがゼロでも回答でき（Q26 は正解、Q28 は部分正解）、Class A の2問（印章指輪、デリー皇帝の嘆願書）ともグラフ探索で自己解決できました。しかし日本語では、コンテキストがゼロで正解した問題は1問もなく、Class A も2問中1問（デリー皇帝の嘆願書）しか回収できませんでした——印章指輪のチェーンはグラフ上に明確に存在するにもかかわらず（別の質問の回答文中に現れている）見逃されています。詳細は [results-ja/README.md § GraphRAG](results-ja/README.md#graphrag) を参照してください。
...
* **上位を分離できる:** Ceiling の6モデル（英語）で、qwen が 100 とした3モデルは Jev では 93〜96 に下がり、qwen3.6（88）との差も広がります。
