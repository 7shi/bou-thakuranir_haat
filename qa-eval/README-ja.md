...
> [!IMPORTANT]
> **実用的な最適解**
> 両言語ともに、**Dense ∪ BM25 (Union)** の組み合わせ（`Hybrid`）が最高精度（英語: **0.960**, 日本語: **0.920**）を達成しており、実用上の最適解です。
...
* **日本語版における k=5 と k=10 のスコア一致:** 日本語版では、`Hybrid k=10` の方がリトリーバの回収率が高い（48/50 vs 43/50）にもかかわらず、最終的なQA精度は `k=5` と `k=10` の双方で同じ **44/50 (0.920)** となりました。不一致分析（Disagreement analysis）によると、`k=10` にすることで Q27 や Q36 などのコンテキスト不足が解消された一方で、コンテキストサイズの肥大化（約13シーンから25シーンへ増加）による「Lost in the middle（中間情報の喪失）」が発生し、Q34 などで回答合成エラー（synthesis error）が生じて失点したため、リトリーバル向上の恩恵と完全に相殺されたことが原因です。
...
3. **最適戦略の言語間での一致:**
   * 両言語ともに、**Dense ∪ BM25 Union** (`Hybrid`) が最善のアプローチです（英語: **0.960**, 日本語: **0.920**）。単一の密ベクトル検索の限界を効果的に打破し、固有名詞や特徴的な用語を回収することで Ceiling に最も近づきました。
