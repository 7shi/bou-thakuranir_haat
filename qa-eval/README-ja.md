...
| Hybrid k=5 | 43/50 (0.910) | 45/50 (0.930) | Dense ∪ BM25 集合和 (k=5) |
| Hybrid k=8 | 46/50 (0.940) | 46/50 (0.950) | Dense ∪ BM25 集合和 (k=8) |
| Hybrid k=10 | 48/50 (0.970) | 45/50 (0.930) | Dense ∪ BM25 集合和 (k=10) |
...
> [!IMPORTANT]
> **実用的な最適解**
> 英語においては `Hybrid k=10` が最高精度 **0.970** を、日本語においては `Hybrid k=8` が最高精度 **0.950** を達成しており、いずれも **Dense ∪ BM25 (Union)** の組み合わせが実用上の最適解です。
...
* **日本語版 k=8 のスイートスポットと英語版 k=10 の比較:** 日本語版では、`k=10` にするとコンテキストの肥大化による回答合成エラー（Q34）が生じるため、`k=8` が **46/50 (0.950)** をマークする最適なスイートスポットとなりました。一方、英語版ではコンテキスト増大による合成エラーの影響が少なく、`Hybrid k=10` が最高精度 **48/50 (0.970)** を維持しており、`Hybrid k=8` は **46/50 (0.940)** となっています。
...
3. **最適戦略の言語間での一致:**
   * 両言語ともに、**Dense ∪ BM25 Union** (`Hybrid`) が最善のアプローチです（英語: **0.970** (k=10), 日本語: **0.950** (k=8)）。単一の密ベクトル検索の限界を効果的に打破し、固有名詞や特徴的な用語を回収することで Ceiling に最も近づきました。
