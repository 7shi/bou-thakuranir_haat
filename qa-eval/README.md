...
- `build_index.py` — scene embedding index (title + text) → `index-<lang>.safetensors`
...
Both retrievers index the scene **title** alongside its text. The titles come
from `all/<lang>-gemini.tsv` (`chapter`, `segment`, `title`), generated by
`scripts/generate_titles.py`; `qa-eval/Makefile` wires them in as
`TSV := $(ROOT)/all/$(LANG)-gemini.tsv`. `build_index.py` embeds each scene
with the EmbeddingGemma document prompt `title: {title} | text: {text}`, and
`bm25.py` tokenizes `"{title} {text}"` as the document. Line-level indexing
(`--line`) has no per-line title and uses `title: "none"` instead. A scene
missing from the TSV is a hard error, so the TSV must cover every scene of the
JSONL.
